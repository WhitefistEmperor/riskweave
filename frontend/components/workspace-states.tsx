      ...
      : status === 'failed'
        ...
        : status === 'queued'
          ...
          : status === 'running'
            ...
            : Circle;
  return (
    <span className={`run-badge badge-${status}`}>
      <Icon size={13} />
      {status}
    </span>
  );
...
export function LoadingState({ label }: { label: string }) {
  return (
    <div aria-busy="true" aria-label={label} className="loading-state">
  ...
  const title =
    api?.status === 401
      ...
      : api?.status === 404
        ...
        : api?.code === 'MALFORMED_RESPONSE'
          ? 'Response could not be read'
          : api?.status === 413
            ? 'Dataset is too large'
            : api?.code === 'BACKEND_UNAVAILABLE'
              ? 'Backend unavailable'
              : 'Request could not be completed';
  const guidance =
    api?.status === 401
      ? 'A valid session is required. Ask the operator to check the development identity configuration; production authentication is not connected.'
      : api?.status === 404
        ? 'The record may no longer exist or may not be accessible to this identity. Return to Investigations to choose an available record.'
        : api?.status === 413
